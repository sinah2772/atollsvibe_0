...
interface ArticlePreviewProps {
  ...
  coverImage: string;
  imageCaption: string;
  content: any; // The article content in JSON format from TipTap
  isBreaking: boolean;
  isFeatured: boolean;
...
const ArticlePreview: React.FC<ArticlePreviewProps> = ({
  ...
  const renderContent = () => {
        ...
        case 'bulletList': {
          return (
            <ul key={index} className="list-disc pl-6 mb-4">
              {node.content?.map((child, childIndex) => renderNode(child, `${index}-${childIndex}`))}
            </ul>
          );
        }
          ...
          return (
            <div key={index} className="my-4">
              <img 
                ...
                onClick={() => {
                  setEnlargedImage(node.attrs?.src || null);
                  setImageAlt(node.attrs?.alt || '');
                  setImageTitle(node.attrs?.title || '');
                }}
              />
              {node.attrs?.title && (
                <p className="text-center text-sm text-gray-600 mt-2">{node.attrs.title}</p>
              )}
            </div>
          );
        }
    ...
    const renderInlineNode = (node: NodeType, key: string): JSX.Element | string | null => {
      if (!node) return null;
      ...
      if (node.text) {
        ...
        if (node.marks && node.marks.length > 0) {
          return node.marks.reduce((content: JSX.Element | string, mark: MarkType) => {
            switch (mark.type) {
              case 'bold':
              ...
              case 'link':
                return <a key={key} href={mark.attrs?.href} className="text-blue-600 hover:underline">{content}</a>;
              case 'highlight':
                return <mark key={key} className="bg-yellow-200">{content}</mark>;
              case 'code':
                return <code key={key} className="bg-gray-100 rounded px-1 py-0.5 font-mono text-sm">{content}</code>;
              default:
                return content;
            }
          }, result);
    ...
    return (
      <div className="prose prose-lg max-w-none">
        {content.content?.map((node: NodeType, index: number) => renderNode(node, index))}
      </div>
    );
  };
